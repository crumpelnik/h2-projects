...
Run **`code/Hydrogen-Figure-Pipeline.ipynb`** with the R kernel. The master notebook executes eight component notebooks in dependency order, each in a separate R kernel. In the repository layout below, the notebooks live in `code/`, while the repository root is used as the project directory for source-data access and generated outputs.
...
│   └── Hydrogen-Figure-Pipeline.ipynb
└── data/
    ├── Anhydrous Ammonia Export.xlsx
    ├── Competitive Industrial Performance.xlsx
    ├── Country Complexity Ranking.xlsx
    ├── Country Risk Spread.xlsx
    ├── Ease of doing business.xlsx
    ├── Freshwater Withdrawal.xlsx
    ├── Governance Score.xlsx
    ├── H2_IEA_21_cleaned_v3.xlsx
    ├── H2_IEA_22_cleaned_v3.xlsx
    ├── H2_IEA_23_cleaned_v3.xlsx
    ├── H2_IEA_24_cleaned_v3.xlsx
    ├── H2_IEA_25_cleaned_v3.xlsx
    └── H2_IEA_26_cleaned_v3.xlsx
```

The nine notebooks are kept in `code/`, while the source Excel workbooks are kept in `data/`. When `H2_PROJECT_DIR` is set to the repository root, the component notebooks write generated RDS, CSV, PDF and TXT outputs to the repository root. These generated outputs are not part of the committed source layout shown above.

**Current path/input note.** The revised `Data-Preparation.ipynb` refers to `Data/...` with a capital `D`, whereas the repository directory is named `data/`. This will fail on case-sensitive file systems unless the notebook paths are changed to `data/...` or the directory is renamed. The notebook also references `RISE.xlsx`, which is not currently present in `data/`; a complete rebuild therefore requires that file to be added (or the preparation code to be adjusted).
...
The revised preparation notebook currently addresses these workbooks as `Data/...` (capital `D`) and reads sheet **`Projects`**. In the repository layout shown above they are stored in `data/`, so the case-sensitivity note above must be resolved before a complete rebuild. The import routine harmonises headers from rows 2–4 and reads project records below row 4. Fields include project reference, status, country, coordinates, electrolyser capacity, technology, electricity source, end uses and expected online year. Workbook names and original Excel row numbers are retained for sample-count investigation.
...
The revised preparation notebook reads eight country-level workbooks from `Data/` (capital `D`), sheet **`Output`**, range `B6:D10000`. In the repository layout shown above, seven are currently committed under `data/`; `RISE.xlsx` is additionally expected by the notebook. The routine takes the country identifier from the first selected column and the indicator value from the third selected column.

| File | Repository status | Use in the pipeline |
|---|---|---|
| `Country Risk Spread.xlsx` | Included | Main macro control and construction of spread-based rating groups. |
| `Governance Score.xlsx` | Included | Governance control in the current primary and competing-risk specifications. |
| `Ease of doing business.xlsx` | Included | Prepared upstream; available as an alternative governance/institutional control. |
| `Anhydrous Ammonia Export.xlsx` | Included | Prepared upstream; outside the current main model controls. |
| `Competitive Industrial Performance.xlsx` | Included | Prepared upstream; outside the current main model controls. |
| `Country Complexity Ranking.xlsx` | Included | Prepared upstream; outside the current main model controls. |
| `Freshwater Withdrawal.xlsx` | Included | Prepared upstream; outside the current main model controls. |
| `RISE.xlsx` | **Not currently included** | Regulatory Indicators for Sustainable Energy score used as a main macro control by the current model specification. |

Country indicators are joined by standardised country codes and are constant across vintages in this implementation. Projects listing multiple countries receive equally weighted averages of the available country values. Source dates, definitions and any proxy-country substitutions belong in the workbook documentation.

### Geographic data

The current repository does **not** contain or require precomputed geographic raster inputs. Project density is calculated directly in `Data-Preparation.ipynb` from project coordinates, separately by annual vintage, using a leave-one-out Gaussian kernel-density calculation with a 50 km bandwidth and geodesic distances.

Supplementary Figure S1 uses medium-resolution Natural Earth country boundaries accessed through `rnaturalearth::ne_countries()`. These geometries are not stored in `data/`; they must be available through the local R environment/package setup when the figure is generated.

## Generated analysis datasets

| File | Contents | Main consumers |
|---|---|---|
| `master_data.rds` | Complete reconstructed project-year panel, including harmonised statuses, observed/reconstructed-row flags, project characteristics and country indicators. | Figure-Sankey and Figure-Regions. |
...
| `cloglog_standardization_parameters.csv` | Means and standard deviations used to standardise the selected numeric analysis covariates prepared upstream. | Interpretation and reproducibility of transformations. |
...
- R with Jupyter's **IRkernel**, registered under the kernel name `ir`. The revised notebook metadata records R **4.5.2**.
...
- The required source workbooks and access to Natural Earth country geometries through `rnaturalearth`. The current data-preparation code does **not** require precomputed raster inputs.
...
For the repository layout shown above, set the repository root as `H2_PROJECT_DIR` and start Jupyter from `code/`:

```bash
export H2_PROJECT_DIR="/absolute/path/to/h2-projects"
cd "$H2_PROJECT_DIR/code"
jupyter notebook
```

Before a complete rebuild, ensure that the `Data-Preparation.ipynb` input paths match the repository's lowercase `data/` directory and that the additional `RISE.xlsx` input referenced by the current notebook is available.

1. Open `Hydrogen-Figure-Pipeline.ipynb` and select the R kernel.
2. `notebook_dir` should resolve to the repository's `code/` directory; `project_dir` should resolve to the repository root via `H2_PROJECT_DIR`.
...
6. Inspect the execution log, output checks and generated sample-count notes in the repository root.

The master passes `H2_PROJECT_DIR` and `H2_GOVERNANCE_CONTROL` to each component kernel. Generated RDS, CSV, PDF and TXT files are written to `project_dir`, which in the setup above is the repository root. Later cells within a component notebook depend on objects created earlier in that notebook.
...
- In the main Figure 2 coefficient chart, the aggregate `any_enduse` coefficient comes from the any-end-use specification; the other displayed coefficients come from the grouped-end-use specification. Figure 3 follows the same split between its any-end-use panel and sector panels.
- In the preparation rules, a project that disappears from a later IEA vintage is classified as cancelled. For the 2026 `Dormant` and `On hold` labels, the notebook carries forward the project's most recent prior non-terminal development stage; these observations are not treated as progression or failure and are right-censored if they remain in that status in the final vintage. The original label is retained in `status_original` and the carried-stage treatment is flagged by `status_prior_stage_carried`.
- The current core controls are project capacity, time in stage, technology and electricity source. The macro controls include country risk spread, RISE, region and Governance Score; the master also supports `Ease_of_doing_business` or `none` as alternative governance settings. Project density is prepared upstream as `project_kde_50km` but is not included in the current primary, competing-risk or delay model control sets.
...
The annual IEA workbooks and seven country-indicator workbooks listed above are currently stored under `data/`. The current preparation notebook additionally expects `RISE.xlsx`, which is not present in the repository at the time of this README update. No precomputed raster inputs are required by the current data-preparation code. A full run generates the analysis RDS objects, figures, CSVs and caption notes in the repository root. Preserve workbook metadata, source dates, cleaning documentation and the R package environment alongside a release. No package lockfile is supplied in the repository.

## License and data attribution

The repository includes an **MIT License** file at the repository root. Consult `MIT License` for the code reuse terms.

IEA project data, country-indicator inputs and Natural Earth geometries retain their respective source attribution and usage terms. Consult the original providers and accompanying workbook metadata, definitions, references and proxy mappings before redistributing or adapting these inputs.
