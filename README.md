...
**Current path/input note.** The committed `Data-Preparation.ipynb` still refers to `Data/...` with a capital `D`, whereas the repository directory is named `data/`. This will fail on case-sensitive file systems unless the notebook paths are changed to `data/...` or the directory is renamed. The notebook also references `RISE.xlsx`, which is not currently present in `data/`; a complete rebuild therefore requires that file to be added (or the preparation code to be adjusted).
...
The current preparation notebook reads eight country-level workbooks from `data/`, sheet **`Output`**, range `B6:D10000`. Seven of those workbooks are currently committed to the repository. The routine takes the country identifier from the first selected column and the indicator value from the third selected column.
...
The current repository does **not** contain or require precomputed geographic raster inputs. Project density is calculated directly in `Data-Preparation.ipynb` from project coordinates, separately by annual vintage, using a leave-one-out kernel-density calculation with a 50 km bandwidth.
